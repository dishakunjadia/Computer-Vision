...
def generate_random_stereogram(im_size: Tuple[int, int, int] = (51, 51, 3), disparity: int = 4) -> torch.Tensor:
  ...
  im_left = torch.zeros(1) 
  im_right = torch.zeros(1) #placeholder, not actual size
  ############################################################################
  # Student code begin
  ############################################################################
  im_left = torch.randint(0, 2, size=im_size, dtype=torch.float32)
  ...
  top_left_x = c_x - block_size[1]// 2

  bott_right_x = top_left_x + block_size[1] 
  top_left_y = c_y - block_size[0]// 2

  bott_right_y = top_left_y + block_size[0]

  
 
  blk = im_right[top_left_y:bott_right_y, top_left_x:bott_right_x]

  shifted_block_x_end = bott_right_x - disparity

  shifted_block_x_start = top_left_x - disparity
  random_fill = torch.randint(0, 2, size=(block_size[0], disparity, channels), dtype=torch.float32)


  im_right[top_left_y:bott_right_y, shifted_block_x_start:shifted_block_x_end] = blk

  gap_start = bott_right_x - disparity
  
  
  gap_end = bott_right_x

  im_right[top_left_y:bott_right_y, gap_start:gap_end] = random_fill

  im_left = im_left.mean(dim=2, keepdim=True).repeat(1, 1, channels)
  im_right = im_right.mean(dim=2, keepdim=True).repeat(1, 1, channels)
