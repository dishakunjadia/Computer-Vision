...
def get_gaussian_kernel(ksize=7, sigma=5) -> torch.nn.Parameter:
    ...
    gaussian_1d = torch.exp(-0.5 * (ax / sigma) ** 2)
    gaussian_2d = torch.ger(gaussian_1d, gaussian_1d)

    gaussian_2d /= gaussian_2d.sum()

    gaussian_2d = gaussian_2d.view(1, 1, ksize, ksize)
...
def get_sobel_xy_parameters() -> torch.nn.Parameter:
    ...
    sobel_filters = torch.cat([sobel_x, sobel_y], dim=0)
    return nn.Parameter(sobel_filters, requires_grad=False)
